...
- 本地行情浏览：查看已保存股票文件，点击生成日线、月线、年线 K 线，支持 MA5/10/20/60、RSI14 和 MACD；
...
页面的“同步最新行情”按钮会通过 easy-tdx 连接通达信行情服务器，先检查每只股票本地文件的最后日期；没有新完成日线时不会重复下载完整历史，有新数据时才补取并追加到 `.day`。同步过程中页面会显示已处理数量、总股票数、剩余数量和进度百分比，完成后显示写入根数、更新文件数、无变化文件数和错误数。全量同步可能需要较长时间，默认不会随容器启动自动全量同步，避免重启时产生不必要的服务器请求。

共享模式下不要让通达信桌面端和容器同时写同一只股票的同一个 `.day` 文件；应用内部已串行化同步与选股任务，跨应用的写入时序仍应由使用者错开。

## 本地行情浏览

“本地行情”页面读取当前 `vipdoc/sh/lday` 和 `vipdoc/sz/lday` 下已经存在的 A 股 `.day` 文件，按市场和股票代码分页展示文件数量、有效区间和最新收盘价。点击股票后会生成图表；日线、月线和年线分别使用日线数据聚合，开盘取周期首日、最高取周期最高、最低取周期最低、收盘取周期末日，成交量和成交额按周期求和。
