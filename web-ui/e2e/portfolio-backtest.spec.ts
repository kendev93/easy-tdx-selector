...
const result = {
  ...
  fitness_filter_enabled: true, fitness_min_score: 75, fitness_min_trades: 5, fitness_max_drawdown: 0.3,
  ...
  ranking_events: [{ date: '2024-01-03', slots_available: 1, ranking_value: 'indicator_three.varo7', candidates: [{ rank: 1, market: 'SZ', code: '000001', score: 9.5, selected: true, fitness_score: 100, fitness_trades: 8, fitness_passed: true, excluded_reason: null }] }],
...
test('user can run a ranked portfolio backtest and inspect replacement candidates', async ({ page }) => {
  ...
  await page.getByTestId('portfolio-fitness-enabled').check()
  ...
  await expect(page.getByTestId('portfolio-results')).toContainText('适配分 ≥ 75')
